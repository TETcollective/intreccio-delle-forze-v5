...
Il Cosmic Microwave Background è la radiazione fossile del Big Bang, 
con anisotropie che riflettono fluttuazioni primordiali.

Nel mio TU-GUT-SYSY:
\begin{itemize}
  \item La materia oscura come modo a bassa energia spiega \( \Omega_\text{DM} \approx 0.27 \) osservato
  \item L'entropia massima della luce primordiale + asimmetria fotoni-barioni 
    modula le fluttuazioni \( \to \) predizione di suppression del potere 
    a multipoli alti (\( l > 2000 \)) e leggero spostamento del primo picco acustico
  \item Contributi topologici primordiali (\( \alpha_\text{top}(\tau) \neq 0 \) durante inflazione) 
    generano polarizzazione in modo B primordiale non standard - 
    segnale curl distinto dal modo B da lensing
\end{itemize}

Predizioni falsificabili:
\begin{itemize}
  \item Eccesso di potere in modo B a \( l \sim 1000 \)-$2000$ 
    (ordine $10^{-7}~\mu\text{K}^2$)
  \item Correlazione tra modo B e sub-strutture DM
  \item Mitigazione della tensione \( H_0 \) grazie a contributo topologico
\end{itemize}
